...
// Execute the query
$selExNameQuery = $conn->query("SELECT * FROM exam_tbl et INNER JOIN exam_attempt ea ON et.ex_id=ea.exam_id WHERE ea.exmne_id='$eid'");
$exam_id=1;
if ($selExNameQuery) {
    $selExName = $selExNameQuery->fetch(PDO::FETCH_ASSOC);
    if ($selExName) {
        $exam_id = $selExName['ex_id'];
        echo $selExName['ex_title'];
    } else {
        echo "No exam found for this examinee.";
    }
} else {
    echo "An error occurred while fetching the exam data.";
}
?>
